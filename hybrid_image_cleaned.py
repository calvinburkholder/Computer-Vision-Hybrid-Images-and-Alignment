...
# ---------------------------------------------------------------------------
# Shared experiment runner (replaces the 6x duplicated menu branches)
# ---------------------------------------------------------------------------
 
def run_alignment_experiment(diff, num1, num2, low_sd, high_sd, weight,
                              max_shift=None, out_path="hybrid.png"):
    """
    Runs one full alignment + hybrid experiment for a single (difficulty, name)
    pair and saves a 6-panel figure: lowA, highB, hybrid (pixel-aligned) big/small,
    hybrid (gradient-aligned) big/small.
 
    Parameters
    ----------
    difficulty : one of "easy", "medium", "large"
    name       : filename shared by dataset/aligned/<name> and dataset/<difficulty>/<name>
    low_sd, high_sd, weight : hybrid_image parameters (all floats)
    max_shift  : search radius for alignment; defaults per difficulty if None
    out_path   : where to save the figure
    show       : if True, also display with plt.show()
 
    Returns
    -------
    dict with the estimated shifts and file path, so results can be
    collected programmatically across a sweep.
    """
    ...
    _, _, hybrid_nms = hybrid_image(imgA, low_sd, alignB_nms, high_sd, weight)
    ...
    fig.suptitle(
        f"{diff}/{num1, num2}  |  pixel dx,dy=({dx_pixel},{dy_pixel})  "
        f"nms dx,dy=({dx_nms},{dy_nms})",
        fontsize=10,
    )
...
if __name__ == "__main__":
    ...
    res = run_alignment_experiment(
        diff=args.diff, num1=args.num1, num2=args.num2,
